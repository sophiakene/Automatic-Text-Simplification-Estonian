...
class Simplify():
    ...
    def __init__(self, sentences):#, parser, truecase_model, lm, conf_model, comp_model):

        '''self.time = ['kuni', 'pärast', 'enne', 'kunagi', 'ükskord', 'alates', 'millal']
        self.concession = ['aga', 'ehkki', 'kuigi', 'siiski', 'ometi', 'ent', 'kuna']
        self.justify = ['sest', 'tõttu', 'seepärast']
        self.condition = ['kui', 'kas']
        self.condition2 = ['või', 'ehk']
        self.addition = ['ja', 'ning']
        ...
        #self.sentences = sentences #list of dictionaries, output from parser
        ...
        #dictionary mit unregelmässigen verbformen und deren lemmata

    #vlt gar nicht nötig
    '''def recover_quotations(self, sent):
        for word, parsing in sent.items():
            #print(parsing.split(' ')[2])
            if parsing.split(' ')[2] == 'Quo':
                sent['""'] = sent.pop(word)
    ...
    def get_information(self, parsed_sentence_dic):
        #print(parsed_sentence_dic)
        head = None
        n = None
        #print(parsed_sentence_dic.items())
    ...
    #delete ant   
    def transformation(self, sent, n, head):#ant, n, head):#, comp=False, justify=False):
        ...
        def get_subj(sent):
            ...
            subjects = []
            for word in sent:
                if '@SUBJ' in sent[word].split(" "): #and -1 = 0 ###wegen ->0 am schluss?
                    subjects.append(word) 
            return subjects

        def get_root(sent): #1 root pro satz
            for word in sent:
        ...
        def conditional(new_sent_list, orig_sent_dic):#b, ant):
                ...
                if orig_sent_dic[word].split(" ")[4] == 'cond':
                    ...
                    if ksi_index > -1:
                        if word[-4:] == 'ksid':
                            subj = get_subj(sent)
                            if subj:
                                pers = sent[subj].split(" ")[4] + sent[subj].split(" ")[5] #function?
                                if pers == 'ps3pl' or 'pl' in sent[subj].split(" "):
                    ...
                    #short conditional form
                        ...
                        #--> make a dic. look in estonian grammar for complete list of irregular verb forms
                        if lemma == 'taht':
                            lemma = 'taha'
                        if lemma == 'pida':
                            lemma = 'pea'
                        ...
                        subj = get_subj(sent)
                        if subj:
                            pers = orig_sent_dic[subj].split(" ")[4] + orig_sent_dic[subj].split(" ")[5]
                            if pers == 'ps1sg':
                                word = lemma + self.verb_suffixes[0]
                            elif pers == 'ps2sg':
                                word = lemma + self.verb_suffixes[1]
                            elif pers == 'ps3sg' or 'sg' in sent[subj].split(" "):
                                word = lemma + self.verb_suffixes[2]
                            elif pers == 'ps1pl':
                                word = lemma + self.verb_suffixes[3]
                            elif pers == 'ps2pl':
                                word = lemma + self.verb_suffixes[4]
                            elif pers == 'ps3pl' or 'pl' in sent[subj].split(" "):
                                word = lemma + self.verb_suffixes[5] 
                            else:
                                word = word
                #ausnahmehandling bisschen schöner bitte
                #more cases like this? look up irregular verb forms esp. 3rd pers.
                if word == 'oleb':
                    word = 'on'
                new_sent += word+' '
            return new_sent
...
##########################################################################################################################################################
        ...
        def relative_clause(sent):#, ant):
            ...
            #eigene funktion falls das noch für andere nebensätze gebraucht wird!
            if demonstrative_index: #aka nume wenns vorher überhaupt es relativpronome gha het  
            ...
            #print(new_sent)
            return new_sent#, demonstrative_index

         

        #MAIN OF TRANSFORMATION
        #return conditional(sent)
        return relative_clause(sent)#, ant)

        #rel = relative_clause(sent, ant)
        #return rearrange_order(rel)
    ...
    def simplify(self, sent, n, head):#, ant, comp=False, conf=False):        
        ...
        simp_sentence_dic = self.transformation(sent, '', n, head)
        
        for word in simp_sentence_dic:
        ...
        simplified_sent_list = self.transformation(sent, n, head) #'' for ant

        simplified_sent = ''
        simplified_sent_list = self.transformation(sent, n, head) #dito
        ...
        if simplified_sent:
            ...
            simplified_sent = simplified_sent.replace(' :', ':')
            simplified_sent = simplified_sent.replace(',.', '.')
            simplified_sent = re.sub('\s+', ' ', simplified_sent) #replaces superfluous white spaces
...
def main():
    ...
    sentences = [{'See': '"see" L0 P dem sg nom cap @SUBJ #1->2', 'on': '"ole" L0 V main indic pres ps3 sg ps af <FinV> <Intr> <0> @FMV #2->0', 'asi': '"asi" L0 S com sg nom @PRD #3->2', ',': '"," Z Com CLB CLB #4->4', 'mida': '"mis" Lda P inter rel sg part @OBJ #5->7', 'ma': '"mina" L0 P pers ps1 sg nom @SUBJ #6->7', 'armastan': '"armasta" Ln V main indic pres ps1 sg ps af <FinV> <Part-P> <InfP> <0> @FMV #7->3', '.': '"." Z Fst CLB #8->8'}, {'Mida': '"mis" Lda P inter rel sg part cap @OBJ #1->3', 'sa': '"sina" L0 P pers ps2 sg nom @SUBJ #2->3', 'armastad': '"armasta" Ld V main indic pres ps2 sg ps af <FinV> <Part-P> <InfP> <0> @FMV #3->0', '?': '"?" Z Int CLB #4->4'}, {'Ma': '"mina" L0 P pers ps1 sg nom cap @SUBJ #1->2', 'küsisin': '"küsi" Lsin V main indic impf ps1 sg ps af <FinV> <NGP-P> <Abl> <0> @FMV #2->0', 'temalt': '"tema" Llt P pers ps3 sg abl @ADVL #3->2', ',': '"," Z Com CLB CLB #4->4', 'mida': '"mis" Lda P inter rel sg part @OBJ #5->7', 'ta': '"tema" L0 P pers ps3 sg nom @SUBJ #6->7', 'armastab': '"armata" Lb V main indic pres ps3 sg ps af <FinV> <0> @FMV #7->3', '.': '"." Z Fst CLB #8->8'}, {'Ta': '"tema" L0 P pers ps3 sg nom cap @SUBJ #1->2', 'vastas': '"vasta" Ls V main indic impf ps3 sg ps af <FinV> <NGP-P> <All> <0> @FMV #2->0', ':': '":" Z Col CLBC CLB #3->3', '': '""" Z Quo CLBC CLB CLO #4->4', 'Ma': '"mina" L0 P pers ps1 sg nom cap CLB @SUBJ #5->6', 'armastan': '"armata" Ln V main indic pres ps1 sg ps af <FinV> @FMV #6->2', 'sind': '"sina" Ld P pers ps2 sg part @OBJ #7->6', '.': '"." Z Fst CLB #8->8', '          ': '""" Z Quo CLB CLC #9->9'}]
    simp_obj = Simplify(sentences)
    #ant = ''
    for sent in sentences:
        #sent_w_quot = simp_obj.recover_quotations(sent)
        n = simp_obj.get_information(sent)[0] #sent_w_quot
        head = simp_obj.get_information(sent)[1]
        x = simp_obj.simplify(sent, n, head) #ant
        print(x)
